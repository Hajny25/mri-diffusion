...
class MemmapDatasetMasked(Dataset):
    def __init__(self, path, path_masks, shape=(1251, 155, 1, 128, 128)):
        self.data = np.memmap(path, mode='r', shape=shape, dtype=np.float32)
        self.masks = np.memmap(path_masks, mode='r', shape=shape, dtype=np.float32)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "neighbor_mask": torch.from_numpy(self.masks[vol_idx, neighbor_idx]),    # mask of slice k-1 or k+1 (clamped at boundaries)
...
def get_debug_dataset():
    return MemmapDatasetMasked(DATA_PATH / "preprocessed_all_debug.npy", DATA_PATH / "preprocessed_all_masks_debug.npy", (10, 155, 1, 128, 128))

def get_full_dataset():
    return MemmapDatasetMasked(DATA_PATH / "preprocessed_all.npy", DATA_PATH / "preprocessed_all_masks.npy", (10, 155, 1, 128, 128))
